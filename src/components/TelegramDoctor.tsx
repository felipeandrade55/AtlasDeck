...
interface Props {
  open: boolean;
  onClose: () => void;
  /** Called after a fix or restart so the parent can refresh ambient state. */
  onChanged?: () => void;
}
...
export function TelegramDoctor({ open, onClose, onChanged }: Props) {
  ...
  const runDiagnose = useCallback(async (sendTest: boolean) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/integrations/telegram/diagnose", {
        method: "POST",
  ...
  useEffect(() => {
    if (open && !data) {
      // first open: run a quiet diagnose (no test message) so we don't spam
      // the user's chat just for opening the doctor
  ...
  const applyFix = useCallback(
    async (check: Check) => {
      ...
      try {
        if (fix.action === "clear-webhook" || fix.action === "drop-pending") {
          const res = await fetch(
            "/api/integrations/telegram?action=clear-webhook",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                accountId: fix.accountId,
                dropPending: fix.action === "drop-pending",
              }),
            },
          );
          const json = (await res.json()) as { success?: boolean; error?: string };
          if (res.ok && json.success) {
            log(true, fix.action === "drop-pending"
              ? "Webhook limpo + pendentes descartados ✓"
              : "Webhook limpo ✓");
            await runDiagnose(false);
            onChanged?.();
          } else {
            log(false, json.error || "Falha ao limpar webhook");
          }
        } else if (fix.action === "restart-gateway") {
          ...
          if (res.ok && json.success) {
            ...
            // Give the gateway a moment to come back, then re-diagnose
          ...
          // The parent owns the setup modal; just close us so the user
          // can click the Setup button. (Could be improved with a
          // callback if both modals get composed at the same level.)
          onClose();
        }
      } catch (e) {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs mt-0.5 opacity-90">
                  {data.testMessage.sent
                    ? "Confere se chegou no seu Telegram. Se não viu, é cliente (notificação silenciada, bot bloqueado)."
          ...
          {data && (["openclaw", "telegram", "config"] as const).map((cat) => {
          ...
          {data && data.summary.fail > 0 && (
            ...
            >
              ...
              <ol className="list-decimal pl-5 mt-1 space-y-0.5">
                <li>Aplique os fixes críticos (botões acima) — webhook hijack é a causa mais comum.</li>
                <li>Mande uma mensagem de teste pra confirmar entrega.</li>
                <li>Se o teste passa mas o Jarvis ainda não responde, reinicie o gateway OpenClaw.</li>
                <li>
                  Se nada disso resolve, no servidor: <code>openclaw doctor</code> (cuidado: pode reverter
                  backups; revise antes de aplicar fixes em massa).
                </li>
              </ol>
            </div>
          )}
