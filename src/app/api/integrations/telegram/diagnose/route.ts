/**
 * GET  /api/integrations/telegram/diagnose
 * POST /api/integrations/telegram/diagnose
 *
 * Thin HTTP wrapper around lib/telegram-diagnostic. The same `runDiagnose`
 * lives in lib so the background health monitor (lib/health-monitor) can
 * call it without going through HTTP.
 */
import { NextRequest, NextResponse } from "next/server";
import { runDiagnose } from "@/lib/telegram-diagnostic";
